...
if @student.save
Enrollment_no = Time.now.year.to_s + "" + @student.standard_id.to_s + "" + @student.id.to_s
...
def list
@students = Student.paginate(: per_page => 4, : page => params[: page])#@ students = Student.search(params[: search]).order(sort_column + " " + sort_direction).paginate(: per_page => 3, : page => params[: page])
end
private
def set_student
